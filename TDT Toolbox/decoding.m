...
for i_decoding = 1:n_decodings % e.g. voxels for searchlight (decoding_subindex in most cases is 1:n_decodings)
    ...
    for i_step = 1:n_steps
        ...
        if strcmp(cfg.results.output, 'corr')
                rows = size(cfg.files.name,1);

                if isequal(current_data(1:rows/2,:), current_data(rows/2+1:rows,:))
                    diag_corr_data = current_data(1:rows/2,:);
                    first_part = ceil(size(diag_corr_data,1)/2);
                    second_part = size(diag_corr_data,1)-first_part+1;
                    runs_shuffled = Shuffle(1:rows/2);
                    co = corrcoef(mean(diag_corr_data(runs_shuffled(1:first_part),:),1), mean(diag_corr_data(runs_shuffled(second_part:end),:),1));
                    decoding_out(i_step) = co(1,2);
                else
                    runs_shuffled = Shuffle(1:rows/2);
                    co = corrcoef(mean(current_data(runs_shuffled(1:rows/4),:),1), mean(current_data((rows/2)+runs_shuffled(rows/4+1:rows/2),:),1));
                    decoding_out(i_step) = co(1,2);
                end
        else
